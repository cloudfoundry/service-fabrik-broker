...
class ServiceFabrikApiController extends FabrikBaseController {
  ...
  getServiceInstanceState(req, res) {
    return Promise
      ...
      .then(() => verifyFeatureSupport(req.plan, 'state'))
    ...
    return Promise
      .try(() => {
        if (trigger === CONST.BACKUP.TRIGGER.SCHEDULED && req.user.name !== config.cf.username) {
          ...
          throw new Forbidden('Scheduled backups can only be initiated by the System User');
          ...
          return this.listBackupFiles(options)
            .then(backupList => {
              ...
              if (onDemandBackups.length >= config.backup.max_num_on_demand_backup) {
                throw new Forbidden(`Reached max quota of ${config.backup.max_num_on_demand_backup} ${CONST.BACKUP.TRIGGER.ON_DEMAND} backups`);
              }
              return true;
            });
        }
  ...
  getRestoreOptions(req, metadata) {
    ...
    return Promise.try(() => req.body.context ? req.body.context : apiServerClient.getPlatformContext({
  ...
  startBackup(req, res) {
    ...
    return Promise
      ...
      .then(() => apiServerClient.updateLastOperationValue({
        resourceGroup: CONST.APISERVER.RESOURCE_GROUPS.DEPLOYMENT,
        resourceType: CONST.APISERVER.RESOURCE_TYPES.DIRECTOR,
  ...
  getLastBackup(req, res) {
    ...
    return Promise
      .try(() => this.setPlan(req))
      .then(() => verifyFeatureSupport(req.plan, CONST.OPERATION_TYPE.BACKUP))
      .then(() => apiServerClient.getLastOperationValue({
        resourceGroup: CONST.APISERVER.RESOURCE_GROUPS.DEPLOYMENT,
        resourceType: CONST.APISERVER.RESOURCE_TYPES.DIRECTOR,
        operationName: CONST.OPERATION_TYPE.BACKUP,
        operationType: CONST.APISERVER.RESOURCE_TYPES.DEFAULT_BACKUP,
      ...
      .catch(NotFound, assert.AssertionError, () => {
    ...
    return Promise
      ...
      .then(backupGuid => {
        return apiServerClient.getResourceState({
          ...
          .then(() => apiServerClient.getResourceOperationStatus({
            resourceGroup: CONST.APISERVER.RESOURCE_GROUPS.BACKUP,
            resourceType: CONST.APISERVER.RESOURCE_TYPES.DEFAULT_BACKUP,
            resourceId: backupGuid,
  ...
  startRestore(req, res) {
    ...
    return Promise
      ...
      .then(metadata => {
        ...
        if (!isRestorePossible(metadata.plan_id, req.plan)) {
      ...
      .then(() => {
        ...
        return apiServerClient.updateLastOperationValue({
          resourceGroup: CONST.APISERVER.RESOURCE_GROUPS.DEPLOYMENT,
          resourceType: CONST.APISERVER.RESOURCE_TYPES.DIRECTOR,
  ...
  getLastRestore(req, res) {
    ...
    return Promise
      ...
      .then(() => {
        return apiServerClient.getLastOperationValue({
          resourceGroup: CONST.APISERVER.RESOURCE_GROUPS.DEPLOYMENT,
          resourceType: CONST.APISERVER.RESOURCE_TYPES.DIRECTOR,
          operationName: CONST.OPERATION_TYPE.RESTORE,
          operationType: restoreType,
  ...
  scheduleBackup(req, res) {
    ...
    return Promise
      ...
      .then(() => {
        ...
        let platform = getPlatformFromContext(req.body.context);
