  ...
  */
  ...
  unlock(resourceId, lockId, maxRetryCount, retryDelay) {
    ...
    return retry(tries => {
      ...
      return apiServerClient.updateResource(opts)
        .tap(() => logger.info(`Successfully unlocked resource ${resourceId} `))
        .catch(Conflict, NotFound, () => {
          logger.info(`Lock on resource ${resourceId} has been updated by some other operation because it expired, no need to unlock now`);
        })
        .catch(err => {
