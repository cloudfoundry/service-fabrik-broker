...
exports.checkQuota = function () {
  return function (req, res, next) {
    if (utils.isServiceFabrikOperation(req.body) || (CONST.HTTP_METHOD.PATCH === req.method && (utils.isNotPlanUpdate(req.body) || utils.isSameSkuUpdate(req.body, catalog)))) {
      logger.debug('[Quota]: Check skipped as it is ServiceFabrikOperation or a normal instance update or plan update with same sku...: calling next handler..');
      next();
    } else {
