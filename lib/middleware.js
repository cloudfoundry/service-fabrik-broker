...
const RiemannClient = utils.RiemannClient;
const config = require('./config');
const DomainSocketClient = utils.DomainSocketClient;
...
exports.checkQuota = function () {
  return function (req, res, next) {
    if (utils.isServiceFabrikOperation(req.body)) {
      ...
      const orgId = req.body.organization_guid || req.body.previous_values.organization_id;   
      if (orgId === undefined) {    
...
exports.requireEventLogging = function (appConfig, appType) {
  ...
  if (appConfig.domain_socket && appConfig.domain_socket.fwd_events) {
    ...
    const domainSockOptions = _
      ...
      .value();
    const domainSockClient = new DomainSocketClient(domainSockOptions);
  }
  const configFileName = `eventlog-config-${appType}.yml`;
  const eventConfigAbsFilePath = process.env.CONF_DIR ? path.join(process.env.CONF_DIR, configFileName) :
    path.join(__dirname, '..', 'config', configFileName);
  const eventsLogInterceptor = new EventLogInterceptor(appConfig.event_type, eventConfigAbsFilePath);
