...
class RiemannClient {
  ...
  handleEvent(message, data) {
    try {
      if (data.event) {
        ...
        if (_.get(config, 'riemann.log_additional_event', true) && typeof data.event.request === 'object') {
          this.suffixGuidsToEventName(data.event);
          this.logEvent(data.event, data.options);
        }
      }
    } catch (err) {
  ...
  suffixGuidsToEventName(eventInfo) {
    ...
    if (instanceId === undefined && backupGuid === undefined && appGuid === undefined) {
      eventName = `${eventName}-guid-not-present-in-request`;
    } else {
