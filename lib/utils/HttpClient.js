...
class HttpClient {
  ...
  getCommand(url, httpMethod) {
    ...
    if (this.baseUrl === undefined || url.indexOf('http') !== -1) {
      ...
      baseUrl = `${urlComponents.protocol}//${urlComponents.hostname}:${urlComponents.port}`;
    ...
    if (this.commandMap[baseUrl]) {
      if (this.commandMap[baseUrl][`${baseUrl}_${httpMethod}_${path}_CIRCUIT`]) {
        return this.commandMap[baseUrl][`${url}_${httpMethod}_CIRCUIT`];
      } else if (this.commandMap[baseUrl][`${baseUrl}_${httpMethod}_CIRCUIT`]) {
        return this.commandMap[baseUrl][`${baseUrl}_${httpMethod}_CIRCUIT`];
