...
function hasChangesInForbiddenSections(diff) {
  ...
  if (!_.isEmpty(forbiddenSections) && !_.includes(forbiddenSections, 'director_uuid')) {
    ...
    if (!_.isEmpty(forbiddenSectionsDiff) || removedJobName) {
      throw new errors.Forbidden(`Automatic update not possible. ${!_.isEmpty(forbiddenSectionsDiff)? 'Detected changes in forbidden sections:' + forbiddenSectionsDiff.join(','): `Job definition removed: ${removedJobName}`}`);
    }
  }
  return false;
}
