...
function deploymentNamesRegExp() {
  return new RegExp(`^(${CONST.SERVICE_FABRIK_PREFIX}(_[a-z]*)?)-([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12})$`);
}

function deploymentNameRegExp(service_subnet) {
  let subnet = service_subnet ? `_${service_subnet}` : '';
  return new RegExp(`^(${CONST.SERVICE_FABRIK_PREFIX}${subnet})-([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12})$`);
}
