...
function hasChangesInForbiddenSections(diff) {
  const forbiddenSections = _
    ...
    .value();
  function isJobRemoved(){
    var jobsRegex = new RegExp('^  jobs'); // this regex is to find the position of jobs section
    var jobsLevelRegex = new RegExp('^  [a-z]+'); // this regex is to find the position of next section at the same level as jobs
    var jobNameRegex = new RegExp('^  [- ] name'); // this regex is to find the position of removal in job name
    var jobStartIndex = _.findIndex(diff, element => {
      return jobsRegex.test(element[0]);
    });
    var jobEndIndex = _.findIndex(diff, element => {
      return jobsLevelRegex.test(element[0]);
    }, jobStartIndex+1);
    var jobNameRemovedIndex = _.findIndex(diff, element => {
      return jobNameRegex.test(element[0]) && _.includes(element[1], 'removed');
    });
    if (jobNameRemovedIndex > jobStartIndex && (jobEndIndex === -1 || jobNameRemovedIndex < jobEndIndex)){
      return true;
    } // this checks if the name removal is present within the jobs section only.
    return false;
  }
  if (!_.isEmpty(forbiddenSections) && !_.includes(forbiddenSections, 'director_uuid')) {
    ...
    if (!_.isEmpty(forbiddenSectionsDiff) || isJobRemoved()){
