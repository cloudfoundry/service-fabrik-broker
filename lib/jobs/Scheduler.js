...
class Scheduler {
  ...
  runJob(jobType, jobDefinition, job, done) {
    return this.preCheckForJobRun()
      .then(() => {
        job.__started_At = new Date();
        return jobDefinition.run(job, done);
      })
      .catch(err => {
        //Precondition not met (ie. system could be in maintenance or any other checks in future)
        //Mark current run as failed and reschedule job & exit the process. [JobScheduler takes care of recreation]
        return jobDefinition
          .runFailed(err, 'Pre-Condition failed', job, done)
          .then(() => this.rescheduleJob(jobType, job))
          .then(() => process.exit(err.code || CONST.ERR_CODES.SF_IN_MAINTENANCE));
      });
  }
    ...
    return Promise.try(() => {
      return maintenanceManager
        ...
        .then((maintenanceInfo) => {
          ...
          logger.info('system in maintenance - ', maintenanceInfo);
          throw ServiceInMaintenance.getInstance(maintenanceInfo);
        });
    });
  }
  ...
  getJob(name, jobType, runAt) {
    return Promise
      .try(() => {
        ...
        const jobName = runAt ? `${name}_${jobType}_${runAt.replace(/\s*/g, '')}` : `${name}_${jobType}`;
  ...
  purgeOldFinishedJobs() {
    ...
    return this
      ...
      .then(() => {
        logger.debug('Purged old Jobs from the System');
      })
      .catch(err => logger.error('Error occurred while purging old finished jobs :', err));
  }
