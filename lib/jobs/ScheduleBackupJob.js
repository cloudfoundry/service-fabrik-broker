...
class ScheduleBackupJob extends BaseJob {
  ...
  static run(job, done) {
    return Promise.try(() => {
        ...
        logger.info(`-> Starting ScheduleBackupJob -  name: ${job.attrs.data[CONST.JOB_NAME_ATTRIB]} - with options: ${JSON.stringify(options)} `);
          ...
          .then(() => {
            ...
            return this
              ...
              .catch(errors.Conflict, (err) => {
                ...
                return retry(() => this.reScheduleBackup(job.attrs.data, job.__started_At), {
                  maxAttempts: 3,
                  minDelay: 500
                }).then(() => {
                  throw err;
  ...
  static reScheduleBackup(jobData, firstAttemptAt) {
    return Promise.try(() => {
      jobData.attempt = jobData.attempt === undefined ? 1 : (jobData.attempt + 1);
      if (jobData.firstAttemptAt === undefined) {
        jobData.firstAttemptAt = firstAttemptAt;
      }
