...
class ScheduleBackupJob extends BaseJob {
  ...
  static run(job, done) {
    return Promise.try(() => {
        ...
        if (jobData.attempt === undefined) {
          jobData.attempt = 1;
          jobData.firstAttemptAt = job.__started_At;
        }
        logger.info(`-> Starting ScheduleBackupJob -  name: ${job.attrs.data[CONST.JOB_NAME_ATTRIB]} - with options: ${JSON.stringify(jobData)} `);
