...
class ScheduleBackupJob extends BaseJob {
  ...
  static deleteEachBackup(backup, options) {
  ...
  static deleteOldBackup(job, instanceDeleted) {
    ...
    return backupStore
      ...
      .map(backup => this.deleteEachBackup(backup, options))
      ...
      .then(deleteResponse => {
        if (config.backup.provider.name === 'azure') {
          return backupStore
            .listBackupsOlderInBetween(options, 3, 4)
            .then(backup => this.deleteEachBackup(backup,
              _
              .chain(options)
