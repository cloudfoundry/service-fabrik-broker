...
const boshDirectorClient = require('../bosh').director;
...
class ServiceInstanceUpdateJob extends BaseJob {
  ...
  static updateInstanceIfOutdated(instanceDetails, planId, operationResponse) {
    ...
    return boshDirectorClient.getDeploymentNameForInstanceId(instanceDetails.instance_id)
    .then(deploymentName => this.getOutdatedDiff(deploymentName, instanceDetails.instance_id, plan)
      .then(diffResults => {
  ...
  static getOutdatedDiff(deploymentName, instanceId, plan) {
    return Fabrik.createManager(plan)
      .then((directorManager) => cloudController.getOrgAndSpaceGuid(instanceId)
          .then(opts => directorManager.diffManifest(deploymentName, opts))
      );
  }
