    ...
    return Promise
      .try(() => {
        if (!skipForbiddenCheck) {
          utils.isChangesPresentInForbiddenSections(diff);
        }
      })
      .then(() => Fabrik
  ...
  static getLastRunStatus(name) {
    ...
    return Repository
      ...
      .then(lastRunDetails => {
        ...
        const diffAfterUpdate = count === 1 ? 'TBD' : (_.get(lastRunList[count - 1], 'response.diff') ||
          _.get(lastRunList[count - 1], 'response.jobStatus.diff', null));
        const response = {
          lastRunAt: lastRunList[count - 1].startedAt,
          diff: {
