...
class ServiceInstanceUpdateJob extends BaseJob {
  ...
  static createOpts(instanceId) {
    return Promise
      .try(() => cloudController.getSpaceGuid(instanceId)
        .then(space_guid => cloudController.getOrgGuid(space_guid)
  ...
  static getOutdatedDiff(deploymentName, instanceId, plan) {
    return Fabrik
      .createManager(plan)
      .then((directorManager) => this.createOpts(instanceId)
