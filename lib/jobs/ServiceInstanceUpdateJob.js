...
class ServiceInstanceUpdateJob extends BaseJob {
    ...
    return this
      ...
      .then(diffList => {
        ...
        return this
          ...
          .finally(() => {
            logger.debug('Instance update done - ', operationResponse);
  ...
  static rescheduleUpdateJob(instanceDetails, trackAttempts) {
    return Promise.try(() => {
      if (trackAttempts) {
        instanceDetails.attempt = instanceDetails.attempt === undefined ? 1 : (instanceDetails.attempt + 1);
        if (instanceDetails.attempt >= config.scheduler.jobs.service_instance_update.max_attempts) {
          logger.error(`Auto udpate for instance ${instanceDetails.instance_id}  has exceeded max configured attempts : ${config.scheduler.jobs.service_instance_update.max_attempts} - ${instanceDetails.attempt}}`);
          return;
        }
      }
      logger.info(`Re-Schedulding InstanceUpdate Job for ${instanceDetails.instance_id} @ ${config.scheduler.jobs.reschedule_delay} - Track : ${trackAttempts} - Attempt - ${instanceDetails.attempt}`);
      return ScheduleManager
