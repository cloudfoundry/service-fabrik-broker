...
class ServiceInstanceUpdateJob extends BaseJob {
  ...
  static run(job, done) {
    return Promise.try(() => {
      ...
      if (_.get(config, 'feature.schedule_update', CONST.OFF) !== CONST.ON) {
  ...
  static getLastRunStatus(name) {
    ...
    return Repository
      ...
      .then(lastRunDetails => {
        ...
        const count = lastRunDetails.length;
        let initialJobRunStatus, idx;
        for (idx = count - 1; idx >= 0; idx--) {
          if (lastRunDetails[idx].data.attempt === 1) {
            initialJobRunStatus = lastRunDetails[idx];
            break;
          }
        }
        logger.debug('Initial run status - ', initialJobRunStatus);
        if (lastRunDetails[count - 1].statusCode === CONST.JOB_RUN_STATUS_CODE.SUCCEEDED) {
          logger.debug('Last run status - ', lastRunDetails[count - 1]);
          return {
            status: CONST.OPERATION.SUCCEEDED,
            diff: {
              before: _.get(initialJobRunStatus, 'response.diff'),
              after: _.get(lastRunDetails[count - 1], 'response.diff')
            }
          };
        } else {
          if (initialJobRunStatus.statusCode === CONST.JOB_RUN_STATUS_CODE.SUCCEEDED) {
            ...
            return {
              status: CONST.OPERATION.IN_PROGRESS,
              diff: {
                before: _.get(initialJobRunStatus, 'response.diff'),
                after: _.get(lastRunDetails[count - 1], 'response.diff')
              }
            ...
            return {
              status: CONST.OPERATION.FAILED,
              message: 'Could not initiate update as Backup process was in-progress',
              diff: {
                before: _.get(initialJobRunStatus, 'response.diff'),
                after: _.get(lastRunDetails[count - 1], 'response.diff')
              }
            ...
            return {
              status: CONST.OPERATION.FAILED,
              message: `${lastRunDetails[count - 1].statusCode} - ${lastRunDetails[count - 1].statusMessage}`,
              diff: {
                before: _.get(initialJobRunStatus, 'response.diff'),
                after: _.get(lastRunDetails[count - 1], 'response.diff')
              }
