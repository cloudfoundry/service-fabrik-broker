...
class ServiceInstanceUpdateJob extends BaseJob {
  ...
  static rescheduleUpdateJob(instanceDetails, trackAttempts) {
    return Promise.try(() => {
      ...
      logger.info(`Schedulding InstanceUpdate Job for ${instanceDetails.instance_id} @ ${config.scheduler.jobs.reschedule_delay} - Track : ${trackAttempts} - Attempt - ${instanceDetails.attempt}`);
