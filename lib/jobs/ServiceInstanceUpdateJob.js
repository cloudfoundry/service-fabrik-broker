  ...
  static getLastRunStatus(name) {
    ...
    return Repository
      ...
      .then(lastRunDetails => {
        ...
        const diffBeforeUpdate = _.get(initialJobRunStatus, 'response.diff');
        if (count === 1 && diffBeforeUpdate !== null) {
          updateInProgress = true;
        }
        const diffAfterUpdate = updateInProgress ? updateInProgressMsg : _.get(lastRunList[count - 1], 'response.diff');
        const response = {
