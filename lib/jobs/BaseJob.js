...
class BaseJob {
  ...
  static runSucceeded(response, job, done, user) {
    logger.info(`Job : ${job.attrs.name} - succeeded: ${JSON.stringify(response)}`);
  ...
  static runFailed(err, response, job, done, user) {
    return Promise.try(() => {
      logger.error(`Job : ${job.attrs.name} - failed. More info: `, err);
      //Update Agenda Job status as failed
      job.fail(err);
      job.save();
      logger.info(`Job -  ${job.attrs.name} - Response : `, response);
