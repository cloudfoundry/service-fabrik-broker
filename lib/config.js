...
const filename = process.env.SETTINGS_PATH || path.join(__dirname, '..', 'config', 'settings.yml');
...
completeDirectorConfig(config.director);
...
if (config.enable_circuit_breaker) {
  ...
  hystrixConfig.init({
    ...
    'hystrix.promise.implementation': global.Promise
  });
...
function completeCircutBreakerConfig(config) {
  const configFileName = 'circut-breaker-config.yml';
