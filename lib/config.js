...
config.services.forEach(service => {
  loadActionScripts(service);
  service.plans.forEach(plan => loadActionScripts(plan));
});
...
function loadActionScripts(actionNode) {
  ...
  function loadActions(typeActions, type) {
    switch (type) {
    case 'js':
      _.forEach(typeActions, (template, actionName) => {
        let actionScriptAbsPath = path.join(__dirname, 'fabrik/actions/js', `${actionName}.js`);
        writeActionFile(template, actionScriptAbsPath);
      });
      break;
    case 'sh':
      _.forEach(typeActions, (phaseTemplate, actionName) => {
        _.forEach(phaseTemplate, (template, phase) => {
          let actionScriptAbsPath = path.join(__dirname, 'fabrik/actions/sh', `${actionName}_${phase}`);
          writeActionFile(template, actionScriptAbsPath);
        });
      });
      break;
    default:
      throw new NotImplemented(`file type ${type} is not supported`);
    }
  }
  const actions = actionNode.actions;
  if (actions) {
    _.forEach(actions, (typeActions, type) => loadActions(typeActions, type));
  }
}
