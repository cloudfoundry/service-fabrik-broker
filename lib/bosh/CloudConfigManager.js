...
class CloudConfigManager extends HttpClient {
        ...
        if (cloudConfigOps.LOCKED) {
          ...
          utils
            ...
            .then(guid => {
              ...
              this.getCloudConfig(directorName, CONST.BOSH_CLOUD_CONFIG.SERVICES_NETWORK_DEFINITION)
  ...
  _updateCloudConfig(lockGuid, cloudConfig, directorName) {
    return Promise.try(() => {
      ...
      if (cloudConfigOps.LOCKED && cloudConfigOps.LOCK === lockGuid) {
        ...
        const directorConfig = this.getConfigByName(directorName);
        const payLoad = {
          type: CONST.BOSH_CONFIG_TYPE.CLOUD,
          name: directorConfig.service_network_cloud_config,
  ...
  getCloudConfig(directorName, name) {
    name = name || CONST.BOSH_CLOUD_CONFIG.DEFAULT;
    const query = {
      type: CONST.BOSH_CONFIG_TYPE.CLOUD,
      name: name,
      latest: true
