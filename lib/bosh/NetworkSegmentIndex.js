...
class NetworkSegmentIndex {
  ...
  static findFreeIndex(deployments, subnet, director) {
    return _.sample(NetworkSegmentIndex.getFreeIndices(deployments, subnet, director));
  }

  static getFreeIndices(deployments, subnet, director) {
    let usedIndices = NetworkSegmentIndex.getUsedIndices(deployments, subnet);
    let freeIndices = [];
    let capacity = NetworkSegmentIndex.capacity(subnet, director.config);
...
function calculateCapacity(subnet, config) {
