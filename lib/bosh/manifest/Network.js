...
class Network {
  constructor(opts) {
    ...
    if (opts.ip_management) {
      assert.ok(_.indexOf(_.values(CONST.IP_MANAGEMENT), opts.ip_management) !== -1, `Invalid ip_management network config ${opts.ip_management} - ${JSON.stringify(_.values(CONST.IP_MANAGEMENT))}`);
    }
    this.ip_management = opts.ip_management || CONST.IP_MANAGEMENT.DEFAULT;
  }
