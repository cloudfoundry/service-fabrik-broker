...
class BoshDirectorClient extends HttpClient {
  ...
  populateCacheEntry(deploymentName) {
    logger.debug('inside populate cache entry:', deploymentName);
    return Promise
      .map(config.directors,
        (directorConfig) =>
        this.getDeploymentByConfig(deploymentName, directorConfig)
        .then(() => this.updateCacheEntry(deploymentName, directorConfig))
        .catch(errors.NotFound, () => logger.debug(`${deploymentName} not found in -`, directorConfig.name)));
  }
  ...
  updateCache(config, deployments) {
    return Promise.try(() => {
      ...
      _.map(deployments, deployment => this.updateCacheEntry(deployment.name, config));
    });
  }

  updateCacheEntry(deployment, config) {
    logger.debug(`inside update cache entry for ${deployment}`);
    logger.debug(this.cache[deployment]);
      if (this.cache[deployment] === undefined){
      logger.info('setting cache for: ', deployment);
      //this.cache[manifest.name] = config;
      this.cache[deployment] = config;
    } else {
      logger.info('setting as duplicate');
      //this.cache[manifest.name] = CONST.ERR_CODES.DEPLOYMENT_NAME_DUPED_ACROSS_DIRECTORS;
      this.cache[deployment] = CONST.ERR_CODES.DEPLOYMENT_NAME_DUPED_ACROSS_DIRECTORS;
    }
  }
  ...
  getDirectorConfig(deploymentName) {
    return Promise.try(() => {
      ...
      logger.debug('cache miss for..', typeof deploymentName);
      if (this.cacheLoadInProgress) {
      ...
      return this
        .populateCacheEntry(deploymentName)
        .then(() => {
          logger.debug('after populating cache entry');
          logger.debug('deployment name is:', deploymentName);
          const cache_val = this.cache[deploymentName];
          logger.debug(`cache value is ${cache_val}`);
          if (cache_val !== undefined) {
    ...
    makeRequest(requestDetails, expectedStatusCode, deploymentName, attempt) {
      return this.getDirectorConfig(deploymentName)
        ...
        .catch(NotFound, (err) => {
          logger.info('error looks like:', JSON.stringify(err));
  ...
  makeRequestWithConfig(requestDetails, expectedStatusCode, directorConfig) {
    //console.log(`director config: ${directorConfig}`);
  ...
  getDeploymentsByConfig(config) {
    return this
      ...
      .tap(deployments => {
        ...
        logger.info(`Updated cache for config - ${config.name}`, deployments);
      });
  }
