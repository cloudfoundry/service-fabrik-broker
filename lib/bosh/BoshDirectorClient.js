...
class BoshDirectorClient extends HttpClient {
  constructor() {
    ...
    this.ready = this.populateCache();
  }

  clearCache(config) {
    if (config) {
      logger.info(`clearing cache for config - ${config.name}`);
      _.each(this.cache, (value, key) => _.get(value, 'name') === config.name ? delete this.cache[key] : '');
    } else {
      this.cache = {};
  ...
  populateCache() {
    logger.debug('Loading Bosh DeploymentName cache... current cached deployments:', _.keys(this.cache));
  ...
  populateCacheEntry(deploymentName) {
    ...
    return Promise
      ...
      .finally(() => {
        ...
        logger.info(`Cache updated for - ${deploymentName}, found in director - ${_.get(this.cache[deploymentName], 'name')}`);
      });
  }
  ...
  updateCacheEntry(deployment, config) {
    const directorConfig = this.cache[deployment];
  ...
  getDirectorConfigFromCache(deploymentName, throwExceptionWhenNotFound) {
    const directorConfig = this.cache[deploymentName];
    if (directorConfig !== undefined) {
  ...
  getDeploymentIps(deploymentName) {
    return this.getDeploymentManifest(deploymentName)
      ...
      .then(manifest => _
        .chain(manifest.instance_groups)
        .map(instance_group => _.map(instance_group.networks, net => net.static_ips))
        .flattenDeep()
        .value()
      );
  }
    ...
    return this.getDeploymentManifest(deploymentName)
      ...
      .then(manifest => {
        ...
        _.each(manifest.instance_groups, (instance_group) => {
          agentJob = _.find(instance_group.jobs, job => job.name === 'broker-agent');
          return !agentJob;
        });
        return agentJob.properties;
      });
  }
