...
class BoshDirectorClient extends HttpClient {
  ...
  makeRequest(requestDetails, expectedStatusCode, deploymentName) {
    return this.getDirectorConfig(deploymentName)
      ...
      .catch(NotFound, (err) => {
        ...
        return Promise
          .map(config.directors, (directorConfig) => this.checkAndUpdateCache(deploymentName, directorConfig)
            .then(() => {
              if (this.cache[deploymentName] === undefined) {
                logger.debug('depoyment name not found after cache refresh')
                throw err;
              }
            })
            .then(() => this.makeRequest(requestDetails, expectedStatusCode, deploymentName))
          );
      });
  }
  ...
  checkAndUpdateCache(deploymentName, config) {
    return this
      .makeRequestWithConfig({
        method: 'GET',
        url: `/deployments/${deploymentName}`
      }, 200, config)
      .then(() => this.cache[deploymentName] = config)
      .catch(() => logger.info(`Deployment not found in ${config.name}`));
  }
