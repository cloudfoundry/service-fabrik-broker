...
class BoshDirectorClient extends HttpClient {
  constructor() {
    ...
    this.isReady = this.populateCache();
  }
  ...
  getDeploymentNameForInstanceId(guid) {
    ...
    return Promise.try(() => {
      const match = _
        ...
        .value();
      if (match.length > 0) {
        logger.silly(`Found deployment in cache ..${match[0]}`);
        return match[0];
      }
  ...
  getCloudConfig(directorName) {;
    return this.makeRequestWithConfig({
      ...
      .then(res => {
        ...
        const cloudConfig = yaml.safeLoad(cloudConfigs[0].properties);
        return cloudConfig;
      });
  }
}
