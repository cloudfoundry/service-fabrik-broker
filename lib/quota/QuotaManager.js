...
class QuotaManager {
  ...
  getAllPlanIdsWithSameSKU(planName, serviceName, serviceCatalog) {
    return Promise.try(() => {
      ...
      const splittedPlanNameWithoutVersionInfo = _.drop(planName.split('-')); // assumption here is that service plan names are versioned, and the format is like <version>-{...}-<tshirt-size>
      splittedPlanNameWithoutVersionInfo.unshift(''); // and skuName will be only -{...}-<tshirt-size>   
      const skuName = _.join(splittedPlanNameWithoutVersionInfo, '-');
