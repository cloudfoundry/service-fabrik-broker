        ...
        return this.isOrgWhitelisted(orgId)
          .then(isWhitelisted => {
            if (isWhitelisted) {
              ...
              const skuName = _.join(_.drop(planName.split('-')), '-'); // assumption here is that service pla names are versioned, and the format is like <version>-<deployment-type>-<tshirt-size> and skuName will be only <deployment-type>-<tshirt-size>
              logger.debug(`SKUName is ${skuName}`);
              const planIdsWithSameSKU = [];
              const service = _.find(catalog.services, ['name', serviceName]);
              _.each(service.plans, (plan) => {
                if (plan.name.endsWith(skuName)) {
