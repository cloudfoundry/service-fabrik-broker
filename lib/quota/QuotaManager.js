...
class QuotaManager {
        ...
        return this.isOrgWhitelisted(orgId)
          .then(isWhitelisted => {
            if (isWhitelisted) {
              ...
              let skipQuotaCheck = _.find(catalog.plans, ['id', planId]).metadata.skip_quota_check;
  ...
  getAllPlanIdsWithSameSKU(planName, serviceName, serviceCatalog) {
    return Promise.try(() => {
      ...
      const skuName = planName.substring(planName.indexOf('-', firstIdx)); // and skuName will be only -{...}-<tshirt-size>
