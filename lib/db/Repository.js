...
class Repository {
  ...
  static insertMany(modelName, dataset, user) {
    if (_.isEmpty(user) || (_.isEmpty(user.email) && _.isEmpty(user.name))) {
      throw new Error('user.email or user.name is mandatory for save operation');
    }
    if (!Array.isArray(dataset)) {
      throw new Error('for bulk insert data set must be an array');
    }
    const Model = Mongoose.model(modelName);
    let updateAuditFields = false;
    if (Model.schema.obj.updatedAt !== undefined) {
      updateAuditFields = true;
    }
    const createdAt = new Date();
    _.each(dataset, data => {
      data.createdAt = createdAt;
      data.createdBy = user.email || user.name;
      if (updateAuditFields) {
        data.updatedAt = createdAt;
        data.updatedBy = user.email || user.name;
      }
    });
      ...
      .then(count => {
        ...
        return searchObj
          ...
          .then(resultList => {
            ...
            logger.silly(`Returning : ${modelName} - List : ${JSON.stringify(result)}`);
            return result;
          });
      });
  }
