...
class Repository {
  ...
  static aggregate(modelName, operatorArray) {
    return Promise.try(() => {
      const Model = Mongoose.model(modelName);
      return Model
        .aggregate(operatorArray);
    });
  }
  ...
  static search(modelName, searchCriteria, paginateOpts) {
    ...
    if (searchCriteria &&
      (searchCriteria.populateOptions || searchCriteria.sortBy || searchCriteria.searchBy || searchCriteria.selectBy)) {
      populateOptions = searchCriteria.populateOptions;
      sortBy = searchCriteria.sortBy;
      searchBy = searchCriteria.searchBy;
      selectBy = searchCriteria.selectBy;
    } else {
    ...
    logger.debug(`Search : ${modelName} with criteria : ${JSON.stringify(searchBy)} - ${paginateOpts}`);
    return Model
      .find(searchBy)
      .count()
      .execAsync()
      .then(count => {
        ...
        let searchObj = Model.find(searchBy, selectBy);
        if (populateOptions) {
        ...
        return searchObj
          ...
          .execAsync()
          .then(resultList => {
            logger.debug(`Retrieved : ${paginateOpts.offset} to ${resultList.length} out of ${count} records for  ${modelName}`);
            if (nxtoffset >= count) {
              nxtoffset = -1;
            }
            const result = {
            ...
            logger.debug(`Returning : ${modelName} - List : ${JSON.stringify(result)}`);
            return result;
          });
      });
  }
