...
class MaintenanceManager {
  ...
  updateMaintenace(progressInfo, state, user) {
    ...
    return Repository
      ...
      .then((model) => {
        ...
        logger.info('found system maintenance :', model);
  ...
  getMaintenaceHistory(offset, records, sortBy, sortOrder) {
    ...
    if (_.isArray(sortBy)) {
      ...
      sortOn = _.map(sortBy, (value, idx) => [value, sortOrder[sortOrder.length ? idx : (sortOrder.length - 1)]]);
    } else {
      sortOn = sortBy === undefined ? ['createdAt', 'asc'] : [sortBy, sortOrder];
    }
    ...
    if (records === undefined || records < 0) {
      records = config.mongodb.max_fetch_count;
    }
    return Repository
