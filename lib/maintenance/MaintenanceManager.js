...
class MaintenanceManager {
  ...
  getDowntimePhase(maintenanceInfo, downTimePhases) {
    return _.find(maintenanceInfo.progress, (info) => {
      for (let x = 0; x < downTimePhases.length; x++) {
        if (_.includes(info, downTimePhases[x])) {
          return true;
        }
      }
      return false;
