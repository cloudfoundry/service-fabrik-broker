...
class MaintenanceManager {
  ...
  getMaintenaceInfo() {
    ...
    return Repository
      .findOne(CONST.DB_MODEL.MAINTENANCE_DETAIL, criteria)
      .then(info => info === null ? null : _.set(_.clone(info), 'broker_update_initiated', this.isBrokerUpdateInitiated(info)));
  }

  isBrokerUpdateInitiated(maintenanceInfo) {
    //Drain initiation indicates that broker will be updated post stop by director
    return _.filter(_.get(maintenanceInfo, 'progress', []), item => _.includes(item, config.broker_drain_message)).length > 0;
  }
  ...
  getLastMaintenaceState() {
    ...
    return this
      ...
      .then(maintenanceDetails => {
        ...
        return _.chain(_.clone(maintenanceDetails.list[0]))
          .set('broker_update_initiated', this.isBrokerUpdateInitiated(maintenanceDetails.list[0]))
          .value();
      });
  }
