...
const BlockStorageClient = require('./BlockStorageClient');
...
class CloudProviderClient {
  constructor(settings) {
    ...
    this.blockstorage = this.constructor.createBlockStorageClient(_
      .chain(this.settings)
      .set('provider', this.provider)
      .value()
    );
  ...
  remove(container, file) {
    ...
    return this.storage
      ...
      .catch(CloudProviderError.Unauthorized, err => {
        logger.error(err.message);
        throw new Unauthorized('Authorization at the cloud storage provider failed');
      })
      .catchThrow(CloudProviderError.NotFound, new NotFound(`Object '${file}' not found`));
  }
  ...
  download(options) {
    return Promise
      ...
      .catchThrow(CloudProviderError.NotFound, new NotFound(`Object '${options.remote}' not found`));
  }
  ...
  static createBlockStorageClient(settings) {
    return BlockStorageClient.createBlockStorageClient(settings).value();
  }
