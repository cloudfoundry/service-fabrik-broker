...
class BackupStore {
          ...
          return this
            ...
            .then(() => {
              logger.info('Deleted backup in service container. Deleting meta info in broker container. \
               Only snapshot info if it is in request.');
              if (options.only_snapshot === 'true') {
                logger.info(`Deleting only snapshotId from metadata for ${filename}`);
                return this.cloudProvider.downloadJson(filename)
                  .then(data => this.cloudProvider.uploadJson(filename, _
                    .chain(data)
                    .omit('snapshotId')
                    .value()));
              } else {
    ...
    return Promise
      .try(() => {
        ...
        if (!options.force && data.trigger === CONST.BACKUP.TRIGGER.SCHEDULED) {
          let retentionStartDate = this.filename.isoDate(moment()
            .subtract(config.backup.retention_period_in_days, 'days').toISOString());
          if (options.only_snapshot === 'true') {
            retentionStartDate = this.filename.isoDate(moment()
              .subtract(3, 'days').toISOString());
          }
        ...
        return this.cloudProvider
          ...
          .each(file => {
            if (options.only_snapshot === 'true') {
              if (_.endsWith(file.name, 'metadata.json')) {
                this.cloudProvider.remove(container, file.name)
              }
            } else {
              this.cloudProvider.remove(container, file.name)
            }
  ...
  listBackupsOlderInBetween(options, daysFrom, daysTo) {
    const iteratees = ['started_at'];
    const backupStartedBefore = moment().subtract(daysFrom, 'days').toISOString();
    const backupStartedAfter = moment().subtract(daysTo, 'days').toISOString();

    function getPredicate(isoDateBefore, isoDateAfter) {
      return function predicate(filenameobject) {
        //backUpStartedBefore defaults to current timestamp as part of isoDate function.
        return _.lt(filenameobject.started_at, isoDateBefore) && _.gt(filenameobject.started_at, isoDateAfter);
      };
    }

    return Promise
      .all([this.filename.isoDate(backupStartedBefore), this.filename.isoDate(backupStartedAfter)])
      .spread((isoDateBefore, isoDateAfter) => this.listBackupFiles(options,
        getPredicate(isoDateBefore, isoDateAfter), iteratees));
  }

  listBackupsOlderThan(options, daysPrior) {
    logger.info(`Listing backups older than ${daysPrior} days`);
    const iteratees = ['started_at'];
...
class Filename {
  ...
  stringify(metadata) {
    ...
    return path.posix.join(
      _.isEqual(this.root, 'root_folder') ? metadata.root_folder : metadata.space_guid,
      operation,
      basename
    );
  }
