...
router.route('/service-fabrik/maintenance')
  ...
  .all(middleware.methodNotAllowed(['GET', 'POST', 'PUT']));
router.route('/cloud-config')
  .get(controller.handler('getSfServicesNetworkConfigAsInCloud'))
  .all(middleware.methodNotAllowed(['GET']));
router.route('/cloud-config/reserveIps')
  .put(controller.handler('reserveIpsInCloudConfig'))
