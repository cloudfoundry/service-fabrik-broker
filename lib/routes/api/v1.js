...
operationRouter.use(controller.handler('verifySpacePermission'));
operationRouter.route('/')
  .get(controller.handler('listLastOperationOfAllInstances'))
  .all(middleware.methodNotAllowed(['GET']));

/* Service Instance Router */
instanceRouter.use(controller.handler('verifySpacePermission'));
...
instanceRouter.route('/schedule_update')
  ...
  .all(middleware.methodNotAllowed(['PUT', 'GET']));

/* Backup Router */
backupRouter.use(controller.handler('verifySpacePermission'));
backupRouter.route('/')
