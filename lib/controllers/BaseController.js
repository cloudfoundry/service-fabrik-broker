...
const CfPlatformManager = require('../fabrik/CfPlatformManager');
const K8sPlatformManager = require('../fabrik/K8sPlatformManager');
...
class BaseController {
  ...
  assignInstance(req, res) {
    ...
    const context = req.body ? req.body.context : undefined;
    const platformManager = !context ? undefined : (context.platform === CONST.CONSUMING_ENVIRONMENT.CF ?
      new CfPlatformManager() : (context.platform === CONST.CONSUMING_ENVIRONMENT.K8S ?
        new K8sPlatformManager() : undefined));
    return this
      .createInstance(instance_id, service_id, plan_id, platformManager)
      .tap(instance => platformManager === undefined ? instance.assignPlatformManager() : Promise.resolve({}))
      .tap(instance => _
        .chain(req)
  ...
  createInstance(instance_id, service_id, plan_id, platformManager) {
    return this.fabrik.createInstance(instance_id, service_id, plan_id, platformManager);
  }
