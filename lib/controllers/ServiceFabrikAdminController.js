...
class ServiceFabrikAdminController extends BaseController {
  ...
  getServiceInstanceBackupSummary(req, res) {
    ...
    if (!moment(req.query.start_time, 'YYYY-MM-DD', true).isValid()) {
      throw new BadRequest('Invalid start date, required format YYYY-MM-DD');
    }
    if (!moment(req.query.end_time, 'YYYY-MM-DD', true).isValid()) {
      throw new BadRequest('Invalid end date, required format YYYY-MM-DD');
    }
  ...
  getScheduledBackupInstances(req, res) {
    ...
    if (req.query.end_time && !moment(req.query.end_time, 'YYYY-MM-DD', true).isValid()) {
      throw new BadRequest('Invalid end date, required format YYYY-MM-DD');
    }
