...
const NetworkSegmentIndex = require('../bosh/NetworkSegmentIndex');
...
class ServiceFabrikAdminController extends BaseController {
      ...
      .then(manager => this.cloudController.getOrgAndSpaceGuid(this.getInstanceId(deploymentName))
        .then(opts => Promise
          .all([
            ...
            manager.diffManifest(deploymentName, opts).then(unifyDiffResult)
          ])
  ...
  getOutdatedDeployments(req, res) {
    function mapDeployment(deployment) {
      return _
        ...
        .set('diff', unifyDiffResult(deployment))
        .set('instance', _
  ...
  serviceInstanceBackupSummary(req, res) {
    ...
    if (!moment(req.query.start_time, 'YYYY-MM-DD', true).isValid() ||
      !moment(req.query.end_time, 'YYYY-MM-DD', true).isValid()
    ) {
      throw new BadRequest('Invalid date, required format YYYY-MM-DD');
    }
    ...
    return BackupReportManager
      .createInstanceBackupRecord(req.params.instance_id, start_time, end_time)
      .then(body => res
        .status(200)
        .send(body));
  }
  ...
  getScheduledBackupInstances(req, res) {
    if ((req.query.start_time && !moment(req.query.start_time, 'YYYY-MM-DD', true).isValid()) ||
      (req.query.end_time && !moment(req.query.end_time, 'YYYY-MM-DD', true).isValid())) {
      throw new BadRequest('Invalid date, required format YYYY-MM-DD\n');
    }
