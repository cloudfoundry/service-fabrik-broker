...
const ipManager = require('../fabrik/IpManager');
const cloudConfigManager = require('../bosh/CloudConfigManager');
const Conflict = errors.Conflict;
const Forbidden = errors.Forbidden;
const BadRequest = errors.BadRequest;

class ServiceFabrikAdminController extends BaseController {
  constructor() {
    super();
  }
  ...
  parseServiceInstanceIdFromDeployment(deploymentName) {
    ...
    if (deploymentNameArray !== undefined && deploymentNameArray.length === 3) {
      return deploymentNameArray[2];
    }
  ...
  getServiceFabrikDeployments() {
    ...
    function extractGuidFromName(deployment) {
      return (deployment.guid = _.nth(DirectorManager.parseDeploymentName(deployment.name), 1));
    }
  ...
  startOobRestore(req, res) {
    return Promise
      .try(() => {
        this.validateUuid(req.body.backup_guid, 'Backup GUID');
        const opts = {
          user: req.user,
  ...
  getSfServicesNetworkConfigAsInCloud(req, res) {
    return cloudConfigManager.getCloudConfig(this.director.constructor.getActivePrimary()[0].name)
      .then(body => body === undefined ? res.status(400).send(`SF cloud config '${BoshDirectorClient.getInfrastructure().segmentation.network_name || 'default'}' not found`) : res.status(200).send(body));
  }

  reserveIpsInCloudConfig(req, res) {
    return ipManager.getIpsToBeReserved()
      .then(ipsToReserve => {
        const updateHandler = (cloudConfig) => {
          const segmentation = BoshDirectorClient.getInfrastructure().segmentation;
          if (cloudConfig === undefined) {
            const networkDefiniton = _.find(BoshDirectorClient.getInfrastructure().networks, (network) => network.name === segmentation.network_name);
            logger.info('cloud config undefined - using network definition ', networkDefiniton);
            if (networkDefiniton === undefined) {
              throw new errors.NotFound(`Network definition not found for ${segmentation.network_name} in config`);
            }
            cloudConfig = {
              networks: [networkDefiniton]
            };
          }
          const serviceNetwork = _.find(cloudConfig.networks, (network) => network.name === segmentation.network_name);
          logger.info('Current service network definition ..', serviceNetwork);
          assert.ok(serviceNetwork, `network definition for '${segmentation.network_name}' not found in cloud config`);
          _.each(serviceNetwork.subnets, subnet => {
            subnet.reserved = subnet.reserved || [];
            if (ipsToReserve[subnet.range]) {
              _.each(ipsToReserve[subnet.range], ip => {
                if (subnet.reserved.indexOf(ip) === -1) {
                  subnet.reserved.push(ip);
                }
              });
              subnet.reserved = _
                .chain(subnet.reserved)
                .map(ipAddr => ip.toLong(ipAddr))
                .sort()
                .map(ipInLong => ip.fromLong(ipInLong))
                .value();
            }
          });
          return [cloudConfig, ipsToReserve];
        };
        logger.info('Active Primary - ', this.director.constructor.getActivePrimary()[0].name);
        return cloudConfigManager.fetchCloudConfigAndUpdate(updateHandler, this.director.constructor.getActivePrimary()[0].name);
      })
      .tap(updateResponse => logger.info('Ip Ranges reserved - :', updateResponse[1]))
      .then(updateResponse => res.status(200).send(updateResponse[0]));
  }
}
