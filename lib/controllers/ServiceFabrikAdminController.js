...
class ServiceFabrikAdminController extends BaseController {
  ...
  reportBackupSummary(req, res) {
    return BackupReportManager
      .createInstanceRecord(moment.utc(req.params.start_time).toDate(), moment.utc(req.params.end_time).toDate())
      .then(body => res
        .status(200)
        .send(body));
  }

  getInstanceIds(req, res) {
    return BackupReportManager
      .getInstanceIds(moment.utc(req.params.start_time).toDate(), moment.utc(req.params.end_time).toDate())
      .then(body => res
        .status(200)
        .send(body));
  }
