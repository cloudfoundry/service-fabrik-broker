...
class ServiceFabrikAdminController extends BaseController {
  ...
  updateDeployment(req, res) {
    ...
    if (allowForbiddenManifestChanges === false) {
      ...
      return this
        ...
        .then(diff => utils.isChangesPresentInForbiddenSections(diff))
  ...
  updateOutdatedDeployments(req, res) {
    ...
    function updateDeployment(deployment) {
      return Promise
        .try(() => {
          utils.isChangesPresentInForbiddenSections(deployment.diff);
        })
        .then(() => self.fabrik
          .createOperation('update', {
  ...
  getDeployment(req, res) {
    ...
    this.createManager(req.query.plan_id)
      .then(manager => Promise
        .all([
          this.director.getDeploymentVmsVitals(deploymentName),
          this.director.getTasks({
            deployment: deploymentName
          }),
          manager.diffManifest(deploymentName).then(unifyDiffResult)
        ])
