...
class ServiceFabrikApiController extends BaseController {
  ...
  getInfo(req, res) {
    let dockerMissingImages = [];
    return docker
      .getMissingImages()
      .then(missingImages => dockerMissingImages = missingImages)
      .catch((err) => {
        logger.info('error occurred while fetching docker images', err);
        dockerMissingImages = ['Error occurred while fetching docker images'];
      })
      .finally(() => {
        res.status(200)
          .json({
            ...
            ready: _.isEmpty(dockerMissingImages),
