...
class ServiceFabrikApiController extends BaseController {
  ...
  verifyTenantPermission(req, res) {
    ...
    return Promise
      .try(() => {
        ...
        const platform = (req.body ? (req.body.context ? req.body.context.platform : undefined) : undefined) ||
          req.query.platform;
        ...
        const space_guid = req.body.space_guid || req.query.space_guid ||
          (platform ? (req.query.tenant_id || req.body.context.space_guid || req.body.context.namespace) : undefined);
        if (space_guid) {
      ...
      .try(() => {
        if (trigger === CONST.BACKUP.TRIGGER.SCHEDULED && req.user.name !== config.cf.username) {
          ...
          const options = {
            ...
            tenant_guid: req.entity.space_guid
          };
  ...
  getLastBackup(req, res) {
    ...
    const tenant_guid = req.entity.platform ?
      ((req.entity.platform === CONST.CONSUMING_ENVIRONMENT.K8S) ?
        req.entity.namespace : req.entity.space_guid
      ) :
      req.entity.space_guid;
    return req.manager
      .getLastBackup(tenant_guid, instance_id, noCache)
      .then(result => res
        .status(200)
  ...
  abortLastBackup(req, res) {
    ...
    const tenant_guid = req.entity.platform ?
      ...
      req.entity.space_guid;
    return req.manager
      .abortLastBackup(tenant_guid, instance_id)
      .then(result => res
  ...
  startRestore(req, res) {
    ...
    const tenant_guid = req.entity.space_guid;
    const instance_id = req.params.instance_id;
  ...
  getLastRestore(req, res) {
    ...
    const tenant_guid = req.entity.platform ?
      ...
      req.entity.space_guid;
    return req.manager
      .getLastRestore(tenant_guid, instance_id)
      .then(result => res
        .status(200)
        .send(result)
      )
  ...
  abortLastRestore(req, res) {
    ...
    const tenant_guid = req.entity.platform ?
      ...
      req.entity.space_guid;
    return req.manager
      .abortLastRestore(tenant_guid, instance_id)
      .then(result => res
        .status(result.state === 'aborting' ? 202 : 200)
        .send({})
      );
  ...
  listBackups(req, res) {
    ...
    options.tenant_guid = req.query.platform ?
      ((req.query.platform === CONST.CONSUMING_ENVIRONMENT.K8S) ?
        req.query.namespace : req.query.space_guid
      ) :
      req.query.space_guid;
    return this.listBackupFiles(options)
      .then(body => res
        .status(200)
        .send(body)
  ...
  deleteBackup(req, res) {
    const options = {
      tenant_guid: req.query.platform ?
        ((req.query.platform === CONST.CONSUMING_ENVIRONMENT.K8S) ?
          req.query.namespace : req.query.space_guid
        ) : req.query.space_guid,
  ...
  scheduleBackup(req, res) {
    ...
    const data = _
      ...
      .set('tenant_guid', req.entity.space_guid)
      .set('plan_id', req.manager.plan.id)
      .set('service_id', req.manager.service.id)
      .value();
    return this.cloudController.getOrgAndSpaceDetails(data.instance_id, data.tenant_guid) // Why all these needed ?
      .then(space => {
        ...
        if (checkUpdateRequired) {
          return req.manager
            ...
            .then(deploymentName => this.cloudController.getOrgAndSpaceGuid(req.params.instance_id)
              .then(opts => req.manager.diffManifest(deploymentName, opts))
