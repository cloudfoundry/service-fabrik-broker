...
class ServiceFabrikApiController extends BaseController {
      ...
      .try(() => {
        if (trigger === CONST.BACKUP.TRIGGER.SCHEDULED && req.user.name !== config.cf.username) {
          ...
          const options = {
            ...
            tenant_id: req.entity.space_guid
          };
  ...
  getLastBackup(req, res) {
    ...
    const tenant_id = req.entity.platform ?
  ...
  startRestore(req, res) {
    ...
    const tenant_id = req.entity.space_guid;
    const instance_id = req.params.instance_id;
  ...
  listBackups(req, res) {
    ...
    options.tenant_id = req.query.platform ?
  ...
  scheduleBackup(req, res) {
    ...
    const data = _
      ...
      .set('tenant_id', req.entity.space_guid)
      .set('plan_id', req.manager.plan.id)
      .set('service_id', req.manager.service.id)
      .value();
    return this.cloudController.getOrgAndSpaceDetails(data.instance_id, data.tenant_id) // Why all these needed ?
      .then(space => {
