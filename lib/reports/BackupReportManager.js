  ...
  static getInstanceBackupSummary(instanceId, startTime, endTime) {
    return Promise
      ...
      .spread((triggeredBackups, resultBackups, lastRunDetail, jobDetail) => {
        ...
        // check max_allowed_no_backup_days_in_row failure count
        const maxAllowedNoBackupDaysInRow = config.reporting.max_allowed_no_backup_days_in_row;
        ...
        while (windowEndIndex < noBackupDays.length) {
          ...
          threeConsecutiveBackupFailureCount += (windowEndIndex - windowStartIndex + 1) >= maxAllowedNoBackupDaysInRow ? (windowEndIndex - windowStartIndex + 2 - maxAllowedNoBackupDaysInRow) : 0;
          windowEndIndex++;
