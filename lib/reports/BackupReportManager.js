        ...
        return Promise.all([
            Promise.try(() => {
              if (jobDetail) {
                ...
                if (startTime <= moment.utc(jobDetail.createdAt).startOf('day').toDate()) {
                  ...
                  return getJobRunDetailCount(currentDay, moment.utc(currentDay).endOf('day').toDate())
                    .then(count => {
                      logger.info('got count ', count);
                      if (!count) {
                        logger.info('No backup scheduled on instance create day ', currentDay);
            ...
            Promise.try(() => {
              if (lastRunDetail && (endTime > lastRunDetail.createdAt)) {
                ...
                return getJobRunDetailCount(moment.utc(endTime).startOf('day').toDate(), endTime)
                  .then(count => {
                    if (!count) {
                      logger.info('instance deleted before backup was taken on ', endTime);
          ...
          .then(() => {
            ...
            for (; currentDay < endTime; currentDay = moment(currentDay).add(1, 'days').toDate()) {
              logger.info('checking for backups on ', currentDay);
              ...
              if (startIndex === endIndex) {
                logger.info('No backup day', currentDay);
