    ...
    return Promise.try(() => {
      if (jobDetail) {
        ...
        if (startTime <= moment.utc(jobDetail.createdAt).startOf('day').toDate()) {
          ...
          return this.getJobRunDetailCount(startTime, moment.utc(startTime).endOf('day').toDate())
            .then(count => {
              if (!count) {
  ...
  static getReportEndTime(instanceId, lastRunDetail, instanceRecord, endTime) {
    return Promise.try(() => {
      if (lastRunDetail && (endTime > lastRunDetail.createdAt)) {
        ...
        return this.getJobRunDetailCount(moment.utc(endTime).startOf('day').toDate(), endTime)
          .then(count => {
            if (!count) {
