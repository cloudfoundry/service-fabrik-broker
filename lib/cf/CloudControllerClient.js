...
class CloudControllerClient extends HttpClient {
  ...
  getRabbitMQServiceInstanceIdWithName(rabbitmq_instance_name, org_guid, space_guid, options){
    ...
    .then((serviceInstance) => {
      ...
      var serviceUrl = serviceInstance.resources[0].entity.service_url;
      return this.getResource(serviceUrl)
      .then((service) => {
          //change 'blueprint' to 'rabbitmq' while deploying in a real landscape
          if(service.entity.label !== 'blueprint'){
              throw new RabbitMQServiceInstanceNameNotFound(rabbitmqInstanceName);
          }
          else{
