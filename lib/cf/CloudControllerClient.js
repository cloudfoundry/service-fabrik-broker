...
class CloudControllerClient extends HttpClient {
  ...
  getPlanGuidFromPlanID(planId) {
    return this.getServicePlans(`unique_id:${planId}`)
      .then(plans => (plans.length > 0 ? _.head(plans).metadata.guid : ''));
  }

  getServiceInstancesWithPlansGuids(commaSeparatedPlanGuids) {
    return this.getServiceInstances({
      qs: {
        q: [`service_plan_guid IN ${commaSeparatedPlanGuids}`]
      }
    });
  }
