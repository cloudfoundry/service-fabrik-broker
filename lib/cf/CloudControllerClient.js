...
const MultiTenantServiceInstanceNameNotFound = errors.MultiTenantServiceInstanceNameNotFound;
...
class CloudControllerClient extends HttpClient {
  ...
  getMultiTenantServiceInstanceIdWithName(instance_name, org_guid, space_guid, service_name, options) {
    return this.getResource(`/v2/spaces/${space_guid}/service_instances?q=name:${instance_name}`, options)
      .then((serviceInstance) => {
        if (serviceInstance.resources.length < 1) {
          logger.debug(`Service instance with name ${instance_name} not found in the space ${space_guid}`);
          throw new MultiTenantServiceInstanceNameNotFound(instance_name, service_name);
        }
        ...
        return this.findServicePlanByInstanceId(serviceInstanceId)
          .then((servicePlan) => {
            ...
            if (catalogPlan.service.name === service_name && catalogPlan.manager.settings.agent !== undefined && catalogPlan.manager.settings.agent.supported_features.indexOf('multi_tenancy') !== -1) {
              return Promise.resolve(serviceInstanceId);
            } else {
              logger.debug(`Service instance with name ${instance_name} was found in the space ${space_guid} but it was not a ${service_name} instance with multi-tenancy feature support.`);
              throw new MultiTenantServiceInstanceNameNotFound(instance_name, service_name);
            }
          });
      });
