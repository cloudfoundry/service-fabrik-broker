    ...
    return this.getResource(`/v2/spaces/${space_guid}/service_instances?q=name:${rabbitmq_instance_name}`, options)
      .then((serviceInstance) => {
        if (serviceInstance.resources.length < 1) {
          logger.debug(`Service instance with name ${rabbitmq_instance_name} not found in the space ${space_guid}`);
        ...
        return this.findServicePlanByInstanceId(serviceInstanceId)
          .then((servicePlan) => {
            ...
            if (catalogPlan.service.name === 'rabbitmq' && catalogPlan.manager.settings.agent !== undefined && catalogPlan.manager.settings.agent.supported_features.indexOf('multi_tenancy') !== -1) {
              return Promise.resolve(serviceInstanceId);
            } else {
              logger.debug(`Service instance with name ${rabbitmq_instance_name} was found in the space ${space_guid} but it was not a rabbitmq instance with multi-tenancy feature support.`);
              throw new RabbitMQServiceInstanceNameNotFound(rabbitmq_instance_name);
            }
          });
      });
