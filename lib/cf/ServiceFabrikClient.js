  ...
  abortLastBackup(options) {
    return this.tokenIssuer
      ...
      .then(accessToken => this
        .request({
          method: 'DELETE',
          url: `/api/v1/service_instances/${options.instance_guid}/backup?space_guid=${options.tenant_guid}`,
          auth: {
            bearer: accessToken
          },
          json: true
  ...
  deleteBackup(options) {
    return this.tokenIssuer
      ...
      .then(accessToken => this
        .request({
          method: 'DELETE',
          url: `/api/v1/backups/${options.backup_guid}?space_guid=${options.tenant_guid}`,
          auth: {
            bearer: accessToken
          },
          json: true
