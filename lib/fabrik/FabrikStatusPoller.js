...
class FabrikStatusPoller {
  ...
  static start(instanceInfo, operation) {
    return Promise
      .try(() => {
        ...
        const checkStatus = () => {
          ...
          DirectorManager
            ...
            .tap(status => operationStatus = status)
            .catch(error => {
              logger.error(`Error occurred while checking ${operation} status of :${instanceInfo.deployment}`, error);
            })
            .finally(() => {
              let operationTimedOut = false;
              logger.info(`Status of ${operation} operation on : ${instanceInfo.deployment} for guid: ${instanceInfo.backup_guid} - `, operationStatus);
              let isOperationFinished = operationStatus && utils.isServiceFabrikOperationFinished(operationStatus.state);
              if (!isOperationFinished) {
                const currentTime = new Date();
                const duration = (currentTime - new Date(instanceInfo.started_at)) / 1000;
                logger.info(`Lock duration : ${duration} / ${config.director.lock_deployment_max_duration} (secs) - for ${operation} on : ${instanceInfo.deployment} for guid : ${instanceInfo.backup_guid}`);
                if (duration > config.director.lock_deployment_max_duration) {
