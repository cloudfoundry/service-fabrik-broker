  ...
  static start(instanceInfoInput, operation, user) {
    return Promise
      .try(() => {
        ...
        assert.ok(instanceInfo.tenant_guid, `${operation} poll operation must have the property 'tenant_guid'`);
                    ...
                    if (operationTimedOut) {
                      ...
                      if (operation === CONST.OPERATION_TYPE.BACKUP) {
                        if (!abortInitiated) {
                          ...
                          return serviceFabrikClient.abortLastBackup(_.pick(instanceInfo, ['instance_guid', 'tenant_guid']));
                        }
