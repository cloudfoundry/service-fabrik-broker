        ...
        const checkStatus = () => {
          ...
          return DirectorManager
            ...
            .finally(() => {
                ...
                if (operationTimedOut) {
                  ...
                  if (operation === CONST.OPERATION_TYPE.BACKUP) {
                    ...
                    if (abortDuration < config.backup.abort_time_out) {
                      logger.info(`backup abort is still in progress since ${abortDuration}(ms) on : ${instanceInfo.deployment} for guid : ${instanceInfo.backup_guid} - Abort timeout after: ${config.backup.abort_time_out}(ms)`);
                      return;
                    } else {
              ...
              if (isOperationFinished) {
                logger.info(`Polling complete. Unlocking Deployment : ${instanceInfo.deployment}`);
