...
class ReserveIpsUseMongo extends BaseAction {
  /* jshint unused:false */
  static executePreCreate(instanceId, deploymentName, reqParams, sfOperationArgs) {
    ...
    return cloudConfigManager.getCloudConfig(directorName, CONST.BOSH_CLOUD_CONFIG.SERVICES_NETWORK_DEFINITION)
  ...
  static reserveIp(cloudConfig, instanceId) {
    return this
      ...
      .then((usedIps) => {
        ...
        assert.ok(serviceNetwork, `network definition for 'sf_bosh_services' not found in cloud config`);
...
module.exports = ReserveIpsUseMongo;