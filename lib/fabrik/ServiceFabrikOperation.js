...
function DeploymentLocked(err) {
  ...
  const httpStatus = _.get(response, 'http.status');
  return description.indexOf(CONST.OPERATION_TYPE.LOCK) > 0 && httpStatus === 409 && response.error_code === 'CF-ServiceBrokerBadResponse';
}
...
class ServiceFabrikOperation {
  ...
  invoke() {
    return this
      ...
      .catch(DeploymentLocked, () => {
        throw new DeploymentAlreadyLocked(this.deployment || this.instanceId);
      });
  }
