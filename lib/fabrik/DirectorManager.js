...
const ActionManager = require('./actions/director/ActionManager');
...
class DirectorManager extends BaseManager {
  ...
  getDeploymentName(guid) {
    let subnet = this.subnet ? `_${this.subnet}` : '';
    return `${DirectorManager.prefix}${subnet}-${guid}`;
  }
  ...
  getDeploymentIps(deploymentName) {
    return this.director
      ...
      .reduce((ipList, vm) => ipList.concat(vm.ips), []);
  }

  createOrUpdateDeployment(deploymentName, params, args) {
    const previousValues = _.get(params, 'previous_values');
    const operation = _.isPlainObject(previousValues) ? CONST.OPERATION_TYPE.UPDATE : CONST.OPERATION_TYPE.CREATE;
    ...
    logger.info(`Starting to ${operation} deployment '${deploymentName}'...`);
    return Promise
      .try(() => {
        switch (operation) {
      ...
      .then(() => operation === CONST.OPERATION_TYPE.CREATE ?
        this.executePreCreateActions(operation, deploymentName, params, args) : Promise.resolve({}))
      ...
      .then(manifest => this.director.createOrUpdateDeployment(operation, manifest, args))
      .tap(taskId => logger.info(`+-> Scheduled ${operation} deployment task '${taskId}'`))
      .catch(err => {
        logger.error(`+-> Failed to ${operation} deployment`);
        logger.error(err);
        throw err;
      });
  }

  executePreCreateActions(deploymentName, params, args) {
    return Promise.try(() => {
      if (this.service.actions || this.plan.actions) {
        const cumilativeActions = this.service.actions ? (this.plan.actions ? `${this.service.actions},${this.plan.actions}` : this.service.actions) :
          this.plan.actions;
        logger.info(`actionsToPerform - @service - ${this.service.actions} , @plan - ${this.plan.actions}`);
        const actionsToPerform = _.chain(cumilativeActions)
          .replace(/\s*/g, '')
          .split(',')
          .value();
        logger.info(`Cumulative actions to perform on ${deploymentName} - ${actionsToPerform}`);
        if (actionsToPerform.length === 0) {
          logger.info(`no actions to perform for ${deploymentName}`);
          return {};
        }
        const preDeployResponse = {};
        return Promise.map(actionsToPerform, (action) => {
            logger.info(`Looking up action ${action}`);
            const actionHandler = ActionManager.getAction(CONST.SERVICE_LIFE_CYCLE.PRE_CREATE, action);
            return actionHandler(this.getInstanceGuid(deploymentName), deploymentName, params, args)
              .tap(resp => preDeployResponse[action] = resp);
          })
          .tap(() => logger.info('Predeploy response ...', preDeployResponse))
  ...
  updateDeployment(deploymentName, opts) {
    return this
      ...
      .then(manifest => this.director
        .createOrUpdateDeployment(CONST.OPERATION_TYPE.UPDATE, manifest, opts)
      );
  }
  ...
  getServiceInstanceState(instanceGuid) {
    return this.getDeploymentIps(this.getDeploymentName(instanceGuid))
