...
const infrastructure = config.director.infrastructure;
...
class DirectorManager extends BaseManager {
  ...
  get stemcell() {
    return _(this.settings)
      .chain()
      .get('stemcell', {})
      .defaults(infrastructure.stemcell)
  ...
  get networkName() {
    return this.subnet || infrastructure.segmentation.network_name || 'default';
  }

  get resourcePools() {
    const networkName = this.networkName;
    const stemcell = this.stemcell;
    return _.reduce(infrastructure.azs, (result, az) => {
      _.forEach(infrastructure.vm_types, vm_type => {
        result.push({
  ...
  getNetworks(index) {
    return new Networks(infrastructure.networks, index, infrastructure.segmentation);
  }
  ...
  generateManifest(deploymentName, opts) {
    ...
    const header = new Header({
      ...
      compilation: infrastructure.compilation,
      disk_pools: infrastructure.disk_types,
  ...
  static get prefix() {
    return config.director.prefix || super.prefix;
  }
