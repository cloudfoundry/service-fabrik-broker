...
class DirectorManager extends BaseManager {
  ...
  executeActions(phase, context) {
    return Promise.try(() => {
      let serviceLevelActions = this.service.actions;
      let planLevelActions = phase === CONST.SERVICE_LIFE_CYCLE.PRE_UPDATE ? catalog.getPlan(context.params.previous_values.plan_id).actions :
        this.plan.actions;
