...
class DirectorManager extends BaseManager {
  ...
  get stemcell() {
    return _(this.settings)
      .chain()
      .get('stemcell', {})
      .defaults(this.director.config.infrastructure.stemcell)
  ...
  get networkName() {
    return this.subnet || this.director.config.infrastructure.segmentation.network_name || 'default';
  }

  get resourcePools() {
    const networkName = this.networkName;
    const stemcell = this.stemcell;
    return _.reduce(this.director.config.infrastructure.azs, (result, az) => {
      _.forEach(this.director.config.infrastructure.vm_types, vm_type => {
        result.push({
  ...
  getNetworks(index) {
    return new Networks(this.director.config.infrastructure.networks, index, this.director.config.infrastructure.segmentation);
  }
  ...
  aquireNetworkSegmentIndex(guid) {
    ...
    return this.getDeploymentNames(true)
      .then(deploymentNames => {
        ...
        return NetworkSegmentIndex.findFreeIndex(deploymentNames, this.subnet, this.director);
      })
  ...
  generateManifest(deploymentName, opts) {
    ...
    const header = new Header({
      ...
      compilation: this.director.config.infrastructure.compilation,
      disk_pools: this.director.config.infrastructure.disk_types,
