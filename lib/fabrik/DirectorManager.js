...
class DirectorManager extends BaseManager {
  ...
  startBackup(opts) {
    ...
    const data = _
      ...
      .assign({
        ...
        finished_at: null,
        tenant_guid: opts.context ? this.getTenantGuid(opts.context) : args.space_guid
      })
      .value();
        ...
        return this.agent
          ...
          .then(() => {
            ...
            const instanceInfo = _.chain(data)
              .pick('tenant_guid', 'backup_guid', 'instance_guid', 'agent_ip', 'service_id', 'plan_id')
              ...
              return this
                ...
                .finally(() => {
                  if (metaUpdated) {
                    const options = _
                      .chain(data)
                      .pick(data, 'tenant_guid', 'backup_guid')
                      .set('force', true)
                      .value();
  ...
  getBackupOperationState(opts) {
    ...
    const options = _.assign({
      ...
      tenant_guid: this.getTenantGuid(opts.context)
    }, opts);
  ...
  getLastBackup(tenant_guid, instance_guid, noCache) {
    return this.backupStore
      .getBackupFile({
  ...
  abortLastBackup(tenant_guid, instance_guid, force) {
    return this.backupStore
      .getBackupFile({
        tenant_guid: tenant_guid,
        service_id: this.service.id,
  ...
  getLastRestore(tenant_guid, instance_guid) {
    return this.backupStore
      .getRestoreFile({
        tenant_guid: tenant_guid,
        service_id: this.service.id,
  ...
  abortLastRestore(tenant_guid, instance_guid) {
    return this.backupStore
      .getRestoreFile({
        tenant_guid: tenant_guid,
        service_id: this.service.id,
  ...
  deleteRestoreFile(tenant_guid, instance_guid) {
    const options = {
      tenant_guid: tenant_guid,
      service_id: this.service.id,
      plan_id: this.plan.id,
