...
class DirectorManager extends BaseManager {
  ...
  createOrUpdateDeployment(deploymentName, params, args) {
    ...
    return Promise
      .try(() => {
        ...
        case CONST.OPERATION_TYPE.UPDATE:
          ...
          return this
            ...
            .then(() => {
              let updateActionContext = _.cloneDeep(actionContext);
              _.chain(updateActionContext)
                .set('instance_guid', this.getInstanceGuid(deploymentName))
                .value();
              _.chain(updateActionContext.params)
                .set('service_id', this.service.id)
                .set('plan_id', this.plan.id)
                .value();
              return this
                .getDeploymentIps(deploymentName)
                .then(ips => this.agent.preUpdate(ips, updateActionContext))
                .tap(response => preUpdateResponse = response.actionResponse)
                .catch(FeatureNotSupportedByAnyAgent, ServiceInstanceNotOperational, err => {
                  logger.debug('+-> Catched expected error of feature \'preUpdate\':', err);
                  return;
                })
                .catch(err => {
      ...
      .then((preDeployResponse) => this.generateManifest(deploymentName, opts, preDeployResponse, preUpdateResponse))
  ...
  generateManifest(deploymentName, opts, preDeployResponse, preUpdateResponse) {
    ...
    const context = new EvaluationContext(_.assign({
      ...
      preUpdateActionResponse: preUpdateResponse
    }, opts.context));
