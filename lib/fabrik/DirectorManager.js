...
class DirectorManager extends BaseManager {
  ...
  executePreCreateActions(deploymentName, params, args) {
    return Promise.try(() => {
      ...
      _.forEach(this.service.actions, (typeAction) => {
        _.forEach(typeAction, (actionTemplate, actionName) => {
          actionsToPerform.push(actionName);
        });
      });
      _.forEach(this.plan.actions, (typeAction) => {
        _.forEach(typeAction, (actionTemplate, actionName) => {
          actionsToPerform.push(actionName);
        });
      });
