...
class DirectorManager extends BaseManager {
  ...
  get networkName() {
    if (this.ipManagement === 'dynamic') {
      return `${BoshDirectorClient.getInfrastructure().segmentation.network_name}_dynamic`;
    }
  ...
  getNetworks(index) {
    return new Networks(BoshDirectorClient.getInfrastructure().networks, index, BoshDirectorClient.getInfrastructure().segmentation, this.ipManagement);
  }
  ...
  aquireNetworkSegmentIndex(guid) {
    ...
    return this.ipManagement === 'dynamic' ? Promise.resolve(0) : this.getDeploymentNames(true)
  ...
  findNetworkSegmentIndex(guid) {
    ...
    return this.ipManagement === 'dynamic' ? Promise.resolve(0) : this
      .director
  ...
  generateManifest(deploymentName, opts) {
    ...
    logger.info('Required networks ..', allRequiredNetworks);
    const tags = {
    ...
    if (networks[this.networkName] === undefined) {
      logger.error(`for ${deploymentName} - subnet ${this.networkName} definition not found among the applicable networks defintion : ${JSON.stringify(networks)}`);
