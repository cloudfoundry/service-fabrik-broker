...
class DirectorManager extends BaseManager {
  ...
  executePreCreateActions(operationType, deploymentName, params, args) {
    return Promise.try(() => {
      const actionsToPerform = _.union(this.service.predeploy_actions, this.plan.predeploy_actions);
      if (actionsToPerform.length === 0) {
        return {};
      }
      const preDeployResponse = {};
      return Promise.map(actionsToPerform, (action) => {
          const actionHandler = ActionManager.getAction(action);
          return actionHandler.executePreCreate(operationType, deploymentName, params, args)
