...
class VirtualHostManager extends BaseManager {
  ...
  createVirtualHost(deploymentName, params, instanceId) {
    ...
    return this.director.getDeploymentIps(deploymentName)
      ...
      .tap(() => logger.info('+-> Created virtual host'));
  }
  ...
  deleteVirtualHost(deploymentName, instanceId) {
    ...
    return this.director.getDeploymentIps(deploymentName)
      .then(ips => this.agent.deleteVirtualHost(ips, instanceId))
      .then(() => this.mapper.deleteVirtualHostRelation(instanceId))
      .tap(() => logger.info('+-> Deleted virtual host'))
  ...
  createBinding(deploymentName, instanceId, binding) {
    ...
    logger.info(`Creating binding '${binding.id}' for deployment '${deploymentName}', virtual host '${instanceId}'...`);
