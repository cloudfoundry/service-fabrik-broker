...
class DirectorInstance extends BaseInstance {
  ...
  initialize(operation) {
    return Promise
      ...
      .tap(() => {
        ...
        if (operation.type === 'delete' && _.includes(this.manager.agent.features, 'backup')) {
          return Promise
            .all([
  ...
  finalize(operation) {
    ...
    return Promise
      .try(() => {
        ...
        case 'create':
          if (operation.context.platform === 'cloudfoundry') {
            return this
  ...
  create(params) {
    ...
    return this
      ...
      .then(taskId => _
        .chain(operation)
        .assign(_.pick(params, 'parameters', 'context')) // Changing it from space_guid to context which would have all env related details.
        .set('task_id', taskId)
        .value()
      );
  }
