...
class DirectorInstance extends BaseInstance {
  constructor(guid, manager, platformManager) {
    super(guid, manager);
    this.networkSegmentIndex = undefined;
    this.platformManager = !platformManager ? undefined : platformManager;
  }

  assignPlatformManager() {
    return this.manager.director.getDeploymentNameForInstanceId(this.guid)
      .then(deploymentName => this.manager.director.getDeploymentProperty(deploymentName, 'platform-context'))
      .then(context => {
        if (context.platform === CONST.CONSUMING_ENVIRONMENT.CF) {
          return new CfPlatformManager(this.guid, this.manager, context);
        } else if (context.platform === CONST.CONSUMING_ENVIRONMENT.K8S) {
          return new K8sPlatformManager(this.guid, this.manager, context);
        }
      })
      .tap(platformManager => this.platformManager = platformManager);
  }
  ...
  initialize(operation) {
    return Promise
      ...
      .tap(() => {
        //TODO: This still has coupling with space guid as it calls ensureTenantGuid. 
        //As part of SF 2.0, we can go to ETCD and fetch the tenant_guid which will 
        //ensure we dont call CF again. For now, we add a check for the feature validation.
        if (operation.type === 'delete') {
          return Promise
            .all([
              this.platformManager.preInstanceDeleteOperations({
                ruleId: this.securityGroupName
              }),
  ...
  deleteRestoreFile() {
    Promise.try(() => {
      if (_.includes(this.manager.agent.features, 'backup')) {
        return this.platformManager.ensureTenantGuid()
  ...
  finalize(operation) {
    ...
    return Promise
      .try(() => {
        if (operation.context.platform === CONST.CONSUMING_ENVIRONMENT.CF) {
          switch (operation.type) {
          case 'create':
            //With SF 2.0 design, security group and other environment specific things 
            //will be executed as part of different controllers. Hence, I think different 
            //cloud specific Manager may not be necessary. In that case, all this code will 
            //do is trigger scheduleAutoUpdate.
            return this.platformManager.postInstanceProvisionOperations({
                ruleId: this.securityGroupName,
                ipRules: this.buildIpRules()
              })
              .then(() => this.manager.director
                .createDeploymentProperty(this.deploymentName, 'platform-context', JSON.stringify(operation.context))
              )
              .tap(() => operation.state === CONST.OPERATION.SUCCEEDED ? this.scheduleAutoUpdate() : {});

          case 'update':
            return this.ensureSecurityGroupExists(this.manager.getTenantGuid(operation.context));
          }
        }
      })
      .catch(SecurityGroupNotCreated, BadRequest, err => _.assign(operation, {
        state: 'failed',
