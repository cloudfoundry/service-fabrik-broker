  ...
  finalize(operation) {
    ...
    return Promise
      .try(() => {
        ...
        case 'create':
          return this.createSecurityGroup(operation.space_guid);
        case 'update':
          return this.ensureSecurityGroupExists();
        case 'delete':
          if (operation.state === 'done') {
            var deploymentName = this.parseTaskid(operation.task_id, 1);
            logger.info(`Deleting cache entry for ${deploymentName}`);
            this.manager.director.deleteCacheEntry(deploymentName);
          }
        }
      })
