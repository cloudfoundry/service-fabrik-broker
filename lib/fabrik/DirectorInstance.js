...
class DirectorInstance extends BaseInstance {
  ...
  scheduleAutoUpdate() {
    //Getting service instance is unneccessary but call is done just to ensure we have the mapping with instance name in case it gets deleted
    const options = {
      instance_id: this.guid,
      repeatInterval: utils.getRandomCronForOnceEveryXDays(_.get(config, 'scheduler.jobs.service_instance_update.run_every_xdays', 15))
    };
    return utils
      .retry(tries => {
