...
class VirtualHostInstance extends BaseInstance {
  ...
  create(params) {
    ...
    return this.cloudController.getServiceInstances({
        qs: {
          q: [`space_guid:${params.space_guid}`, `name:${params.parameters.dedicated_rabbitmq_instance}`]
        }
      })
      .then(serviceInstances => {
        if (serviceInstances.length < 1) {
          logger.error(`Service instance with name ${rabbitmqInstanceName} not found in the space ${params.space_guid}`);
          throw new NotFound(`Service instance with name ${rabbitmqInstanceName} not found`);
        }
        return serviceInstances[0].metadata.guid;
      })
      .then(serviceInstanceId => this.director.getDeploymentNameForInstanceId(serviceInstanceId))
