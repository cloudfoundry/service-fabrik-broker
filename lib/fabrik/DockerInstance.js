...
class DockerInstance extends BaseInstance {
  ...
  get platformContext() {
    return this.inspectContainer()
      .then(() => {
        if (this.getEnvironment().context) {
          return JSON.parse(this.getEnvironment().context);
        } else {
          throw new NotFound(`platform-context not found in docker environemt for instance ${this.guid}`);
        }
      })
      .catch(NotFound, () => {
      ...
      .catch(err => {
        if (err.statusCode === CONST.HTTP_STATUS_CODE.NOT_FOUND ||
