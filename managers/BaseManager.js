...
class BaseManager {
  ...
  registerWatcher(resourceGroup, resourceType, queryString) {
    ...
    return eventmesh.apiServerClient.registerWatcher(resourceGroup, resourceType, this.handleResource, queryString)
      .then(stream => {
  ...
  _preProcessRequest(objectBody, processingLockStatus) {
    ...
    return Promise.try(() => {
      if (!objectBody.metadata.annotations || objectBody.metadata.annotations.lockedByManager === '') {
        return BaseManager.prototype._acquireProcessingLock(objectBody)
          .catch(err => {
  ...
  _postProcessRequest(objectBody) {
    const options = JSON.parse(objectBody.spec.options);
    return BaseManager.prototype._releaseProcessingLock(objectBody)
  ...
  handleResource(changeObject) {
    ...
    if (changeObjectBody.metadata.annotations && changeObjectBody.metadata.annotations.lockedByManager === config.broker_ip) {
      ...
      return BaseManager.prototype._preProcessRequest(changeObjectBody, processingLockStatus)
        .then(() => {
          if (!processingLockStatus.conflict) {
            return this.processRequest(changeObject);
          }
        })
        .catch(err => {
          if (!processingLockStatus.conflict) {
        ...
        .finally(() => {
          if (!processingLockStatus.conflict) {
            return BaseManager.prototype._postProcessRequest(changeObjectBody);
          }
        });
    }
  }
