...
class ScheduleManager {
  ...
  static runNow(name, jobType, jobData, user) {
    let agendaJob = {};
    return scheduler
      .runNow(name, jobType, jobData)
      .tap(job => agendaJob = job)
      .then(() => this.saveJob(name, jobType, 'now', jobData, user, true))
  ...
  static schedule(name, jobType, interval, jobData, user) {
    return Promise.try(() => {
      ...
      if (interval === CONST.SCHEDULE.DAILY) {
        ...
        const isSafe = isCronSafe(interval);
        if (!isSafe) {
          throw new BadRequest(`invalid interval ${interval}. Safety validation failed. `);
        }
      }
      return scheduler
  ...
  static getRandomHourlySchedule(interval) {
    try {
      ...
      if (24 % everyXhrs === 0) {
        ...
        return getRandomCronForEveryDayAtXHoursInterval(everyXhrs);
      } else {
        return interval;
      }
    } catch (err) {
      throw new BadRequest(`invalid interval ${interval} - hours must be valid integer`);
    }
  }
  ...
  static getSchedule(name, jobType) {
    ...
    return scheduler
      ...
      .then(job => {
        if (job === null) {
          throw new NotFound(`Schedule not found for instance ${name} for job type ${jobType}`);
        }
  ...
  static setupSystemJobs() {
    const systemJobDefinition = _.get(config, 'scheduler.system_jobs');
    return Promise.map(systemJobDefinition, jobDefinition => {
      ...
      return this
        ...
        .catch(NotFound, () => scheduleSystemJob = true)
        .finally(() => {
