  ...
  static run(job, done) {
    return Promise.try(() => {
      ...
      if (!_.get(jobData, 'instance_id') || !_.get(jobData, 'type')) {
        ...
        logger.error(msg);
        return this.runFailed(new BadRequest(msg), undefined, job, done);
      }
      ...
      return this
        ...
        .catch(error => {
          return this.runFailed(error, backupRunStatus, job, done)
            .then(() => {
              if (error instanceof Conflict || error instanceof UnprocessableEntity) {
    ...
    return Promise.try(() => {
      ...
      if (jobData.attempt > MAX_ATTEMPTS) {
        ...
        return ScheduleManager.schedule(
          ...
          CONST.SYSTEM_USER)
          .then(() => {
            throw new toManyAttempts(config.scheduler.jobs.scheduled_backup.max_attempts, new Error(`Failed to reschedule backup for ${jobData.instance_id}`));
          });
      }
